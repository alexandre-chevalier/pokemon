...
class Combat:
    ...
    def display_status_effects(self, pokemon, x, y):
        if pokemon.status_effects:
            ...
            status_text = font_path.render(f"Status: {effects_text}", True, YELLOW)
    ...
    def display_characteristics(self):
        ...
        pokemon1_info = font_path.render(
        ...
        pokemon2_info = font_path.render(
    ...
    def attack(self, attacker, target, move=None):
        ...
        if random.random() < move.accuracy:
            ...
            damage_text = font_path.render(f"{attacker.name} deals {damage} damage to {target.name}!", True, RED)
    ...
    def draw_attack_button(self):
        ...
        # Button text
        attack_button_text = font_path.render("Attack", True, BLACK)
    ...
    def display_turn(self, attacker_name):
        turn_info = font_path.render(f"{attacker_name}'s turn to attack!", True, WHITE)
        self.screen.blit(turn_info, (50, 150))
    
    def display_winner(self, winner):
        winner_text = font_path.render(f"Winner: {winner.name}!", True, YELLOW)
    ...
    def record_winner(self, winner, looser):
        ...
        if winner.experience >= winner.limitXP:
            ...
            message3 = font_path.render(f"{winner.name} monte au niveau {winner.level} !", True, YELLOW)
    ...
    def display_end_message(self, winner, looser):
        ...
        message1 = font_path.render(f"{looser.name} is K.O. !", True, RED)
        message2 = font_path.render(f"{winner.name} gagne {looser.giveXp} XP ! XP total: {winner.experience}/{winner.limitXP}", True, WHITE)
        
        message4 = font_path.render(f"{winner.name} a été mis à jour dans players.json", True, WHITE)
        message5 = font_path.render(f"The winner is {winner.name}!", True, YELLOW)
